...
散列函数（哈希函数）：Addr=H（key）建立了“关键字”到“存储地址”

![image-20251023194436134](/coding_images/image-20251023194436134.png)

冲突（碰撞）：在散列表中插入一个数据元素时，需要根据关键字的值确定其存储地址，若改地址已经存储了其他元素，则这种情况被称为“冲突”

同义词：若不同的关键字通过散列函数映射到同一个存储地址，则称他们为“同义词”

拉链法：把同义词存储在一个链表里

开放定时法：发生冲突时，就给新元素找另一个空闲位置。

#### 构造

除留余数法、直接定址法、平方取中法，数字分析法
...
3.为了尽可能减少冲突。散列函数计算出来的地址应尽可能均匀分布在整个地址空间

4.散列函数尽量简单，能够快速计算出任意一个关键字对应散列地址

①.除留余数法

H（key）=key%p

若散列表的长度为m，则取一个不大于m但是接近m的质数作为p

==对质数取余，可以分布更加均匀，减少冲突（非质数可以容易冲突）==

②直接定址法

H（key）=key 或者H（key）=a*key+b

a,b均为常数，不会冲突但是容易浪费空间

![image-20251023210454204](/coding_images/image-20251023210454204.png)

③数字分析法

**选取编码分布较为均匀的若干作为散列地址**

适用于：关键字集合已知，且关键字的某几个数码分布均匀

![image-20251023210801058](/coding_images/image-20251023210801058.png)
...
![image-20251023211027723](/coding_images/image-20251023211027723.png)
...
![image-20251024215122821](/coding_images/image-20251024215122821.png)

构造普遍四种方法

![image-20251024215335770](/coding_images/image-20251024215335770.png)

di：第i次冲突时偏移量

双散列法有两个散列函数，冲突时计算第二个散列函数

![image-20251025105155140](/coding_images/image-20251025105155140.png)

伪随机序列则认为这是一个偏移序列，依次偏移判断是否冲突

**如何删除一个函数：**

1.先跟根据散列函数算出散列地址，并对比关键字是否匹配。若匹配则查找成功
...
![image-20251025110106457](/coding_images/image-20251025110106457.png)

![image-20251025110135095](/coding_images/image-20251025110135095.png)

![image-20251025110214137](/coding_images/image-20251025110214137.png)

==只进行逻辑删除（进行标记），而不进行物理删除。==

拓展：探测覆盖率

![image-20251025110527499](/coding_images/image-20251025110527499.png)

![image-20251025110740335](/coding_images/image-20251025110740335.png)

要表长m为质数即可

#### 散列表性能分析

![image-20251025111326178](/coding_images/image-20251025111326178.png)

![image-20251025111918046](/coding_images/image-20251025111918046.png)

分母应该以散列函数的取值范围为准，而不是以表长为准

![image-20251025112348577](/coding_images/image-20251025112348577.png)
...
![image-20251025112459160](/coding_images/image-20251025112459160.png)

**聚集堆积现象**

利用线性探测法处理冲突时，若出现几个初始散列地址争夺同一个后继散列地址的，则称为“聚集”

==使用平方探测法可以减少聚集现象==

![image-20251025114611918](/coding_images/image-20251025114611918.png)

## 排序

### 基本概念

`排序算法的稳定性`：若排序表中有两个元素，关键字相同，那么排序算法处理之后，他们的相对位置不改变，此算法即为稳定的，否则不稳定。
...
思想：每次将一个待排序的记录按照其关键字大小到前面已排好序的子序列中，直到全部记录插入完成。
...
注意：一直到low>high时才停止折半查找，当mid所指元素等于当前元素时，应继续令low=mid+1,以保证“稳定性”，最终应将当前元素插入到low所指的位置（即high+1）

时间复杂度还是O（n平方）

练习：对链表进行插入排序

![image-20251025122222707](/coding_images/image-20251025122222707.png)

### 希尔排序

先追求表中元素部分有序，然后再逐渐逼近到全局有序

**算法：希尔排序先将待排序表分割成若干形如L[i,i+d,i+2d,.,i+kd]的“特殊”子表，对各个子表**
**分别进行直接插入排序。缩小增量d，重复上述过程，直到d=1为止。**

![image-20251025141346919](/coding_images/image-20251025141346919.png)

第一趟把相距距离为4的放在一个子表里，然后对每一个子表排序，第二趟把相距为2的放在一个子表里，然后再对每一个子表排序，一直到1
...
时间复杂度：与增量序列d的选择有关，所以无法用数学证明，最坏复杂度为o(n平方)

![image-20251025143935670](/coding_images/image-20251025143935670.png)
...
![image-20251025150837743](/coding_images/image-20251025150837743.png)

![image-20251025150903382](/coding_images/image-20251025150903382.png)

### 快速排序

算法思想：算法思想：在待排序表L[1.n]中任取一个元素pivot作为枢轴（或基准，通常取首元素），通过一趟排序将待排序表划
...
**用第一个元素把待排序序列“划分”为两个部分。左边更小，右边更大。该元素的最终位置确定，再对其左子表和右子表进行同样的操作排序**
...
void quicksort(int a[], int left, int right) {
    ...
    int i = left, j = right;
    int pivot = a[left]; // 选第一个元素为基准

    while (i < j) {
        while (i < j && a[j] >= pivot) j--; // 从右往左找小于 pivot 的
        while (i < j && a[i] <= pivot) i++; // 从左往右找大于 pivot 的
        if (i < j) { // 交换
            int temp = a[i];
            a[i] = a[j];
            a[j] = temp;
...
空间复杂度：O（递归层数）
  ...
  ![image-20251026165032833](/coding_images/image-20251026165032833.png)

稳定性：不稳定

![image-20251026165150130](/coding_images/image-20251026165150130.png)
...
每一趟在待排元素中选取关键字最小的元素加入有序子序列(放到最前面)
...
若n个关键字序列工[1.n]满足下面某一条性质，则称为堆
(Heap):
若满足：L()>L(2i)且L(i)>L(2i+1)  大根堆（大顶堆）
若满足：L(i)<L(2i)且L(i)<L(2i+1)  小根堆（小顶堆）

![image-20251028113954082](/coding_images/image-20251028113954082.png)

简单来说：

`大根堆`：完全二叉树中，所有根节点都大于其左右子树的所有节点

`小根堆`：完全二叉树中，所有根节点都小于其左右子树的所有节点

建立大根堆：把所有的非终端节点都检查一遍，看看是不是比左右孩子都大，如果不满足，就进行调整（与更大的孩子进行互换），更小的元素下坠后，可能下坠元素不满足要求，则继续替换，使小元素不断下坠。
...
void HeadAdjust(int A[],int k,int len){
    A[0]=A[k]; //暂存
    for(int i=2*k;i<=len;i*=2){ //沿较大的子节点向下筛选
        if(i<len&&A[i]<A[i+1])
            i++; //找到较大的子节点的下标
        if(A[0]>A[i])
            break;//如果根节点比较大，无事发生跳出循环
        else{//否则替换根节点，将k赋值为i继续进入循环
            A[k]=A[i];
            k=i;
    ...
    A[k]=A[0];//满足大根堆后，把原来的数塞回去
}
```

以上是对于单个元素的大根堆处理，处理完成后排除在排序算法之外，继续排其他元素
...
堆排序：每一趟将堆顶元素加入有序子序列，并将待排序元素序列再次调整为大根堆（使小元素不断下坠）

算法性能分析：

建堆：

![image-20251028121737465](/coding_images/image-20251028121737465.png)

排序：

![image-20251028121823922](/coding_images/image-20251028121823922.png)

![image-20251028121839007](/coding_images/image-20251028121839007.png)

**稳定性：堆排序不稳定**

![image-20251028121946224](/coding_images/image-20251028121946224.png)

### 归并排序

**归并：把两个或多个已经有序的序列进行合并**

![image-20251026213810143](/coding_images/image-20251026213810143.png)

核心操作：把数组中两个有序的序列合并为1个
...
![image-20251026223533721](/coding_images/image-20251026223533721.png)

![image-20251026223722914](/coding_images/image-20251026223722914.png)

### 基数排序

![image-20251026224448632](/coding_images/image-20251026224448632.png)

依照个十百千万的顺序，进行收集，进入10个队列，然后依次按顺序出队



![image-20251026224528031](/coding_images/image-20251026224528031.png)

![image-20251026224432972](/coding_images/image-20251026224432972.png)

过程：

初始化：设置r个空队列，按照各个关键字位权重递增的次序，对d个关键字做“分配”，“收集”

分配：顺序扫描各个元素，若当前处理的的关键字位=x，将元素插入Qx队尾

收集：把Qr-1、Qr-2....Q0各个队列中的结点依次出队并链接

因为要r个辅助队列，空间复杂度为O(r)

一趟分配O(n),一趟收集O(r),总共d趟分配和收集，时间复杂度：       O（d(n+r)）

先进先出稳定性分析：队列，所以该算法是稳定的

**基数排序擅长解决的问题：
①数据元素的关键字可以万便地拆分为d组
且d较小（5个人的身份证号，就不适合，需要18组）
②每组关键字的取值范围不大，即r较小（反例：给中文人名排序）
③数据元素个数n较大**

![image-20251027150715172](/coding_images/image-20251027150715172.png)

### 计数排序

前提：待排序的元素的关键字是整数型，元素取值为[0,k）

![image-20251027212508323](/coding_images/image-20251027212508323.png)

![image-20251027212629244](/coding_images/image-20251027212629244.png)

**核心思想：用额外数组记录每个数出现的次数，再按次数顺序还原出有序序列**

```c
//计数排序
//A为输入数组，B为输出数组，C为辅助数组，刚开始记录每个数出现了多少次，通过处理变为记录小于等于每个数有多少个，最终根据C数组进行还原排序
void CountSort(int A[],int B[],int n,int k){
    ...
    for(i=0;i<n;i++)
        C[A[i]]++;//记录每个数出现了多少次
    for(i=1;i<k;i++)
        C[i]=C[i]+C[i-1];//记录小于等于A[i]的数有多少
    for(i=n-1;i>=0;i++){
        C[A[i]]=C[A[i]]-1;//每还原一个就减一
        B[C[A[i]]]=A[i];//还原
    }
}
```
...
空间复杂度：O（n+k）n来自于数组B，k来自于数组C

时间复杂度：O（n+k）

若k的数量级为n，则总的时间复杂度为n，由于快速排序，堆排序等

但是：![image-20251027230208588](/coding_images/image-20251027230208588.png)

空间换时间

![image-20251027230744715](/coding_images/image-20251027230744715.png)

### 外部排序

#### 外存和内存的数据交换

==内存和外存之间的交互数据读写是以块作为单位的==

![image-20251027231110585](/coding_images/image-20251027231110585.png)

#### 外部排序原理

数据元素太多，无法一次全部存入内存进行排序，使用“归并排序”的方法，最少秩序在内存中分配3块大小的缓冲区即可对任意一个大文件进行排序，归并排序要求各个子序列有序，每次进行内部排序之后写回磁盘。

以上操作叫做初始“归并段”

 ==这一块复习的时候最好看动画理解，不太好用语言描述==

![image-20251027231919061](/coding_images/image-20251027231919061.png)

输入缓冲区空的时候，就继续从磁盘读入一段内容到输入缓冲区中

![image-20251027232253572](/coding_images/image-20251027232253572.png)
...
![image-20251027232353697](/coding_images/image-20251027232353697.png)

减少了归并趟数，从而减少了磁盘IO（读写）次数

![image-20251027232514239](/coding_images/image-20251027232514239.png)

![image-20251027232547924](/coding_images/image-20251027232547924.png)

k越大，r越小，归并趟数越小，读写次数越小

结论：如果能增加初始归并段的长度，就可以减少初始归并段的数量r

![image-20251027232904991](/coding_images/image-20251027232904991.png)

![image-20251027233038405](/coding_images/image-20251027233038405.png)

### 败者树

定义：可视为一棵完全二叉树。k个叶子结点分别是当前参加比较的元素，非叶子结点用来记忆左右子树的“失败者”而让胜者往上进行继续比较，一直到根节点。

![image-20251028110537900](/coding_images/image-20251028110537900.png)

**如果把天津饭换成派大星，根节点的右子树就不用继续比了，只需比较左子树**

![image-20251028111602735](/coding_images/image-20251028111602735.png)

选归并段中最小元素的过程可以大大简化对比次数

### 置换—选择排序

非常好，这个问题问得更深入了。你说的“**置换–选择排序（Replacement Selection Sort）**”，确实不是一般意义上的选择排序，而是**外部排序（External Sorting）\**中——在\**生成初始归并段（runs）\**时使用的一种算法。下面我用\**简洁明白的语言**解释：

------

🧩核心思想：

置换–选择排序是一种**在内存缓冲区中尽量生成更长初始归并段**的算法。
 它的目标是：

> 在有限内存中，让每次输出的归并段尽可能长，从而减少后续归并的次数。

------

⚙️基本过程：

1. **先从磁盘读入内存**中能放下的若干记录（假设内存能放下 M 条）。
2. **选出当前最小的记录输出**到第一个归并段（写回磁盘）。
3. **再从磁盘读入一条新记录**补进内存。
   - 如果这条新记录的关键字 **≥ 上一次输出的关键字**，说明它还能继续排在当前归并段中；
   - 如果 **更小**，则标记为“下一段”的记录，不再参与当前段的选择。
4. 当内存中所有记录都被标记为“下一段”时，说明当前归并段结束。
5. 继续生成下一个归并段，如此反复。
...
- **生成的归并段比内存容量更长**（平均可达约 2M 条记录）。
- **充分利用内存**，减少外部归并的趟数。
- 本质上是**利用堆排序（优先队列）思想**实现的选择过程。

------

📌一句话概括：

> 置换–选择排序是在外部排序中，用堆不断“置换并选择”记录，使生成的初始归并段尽可能长的一种方法。

------

![image-20251028112710591](/coding_images/image-20251028112710591.png)

![image-20251028112734368](/coding_images/image-20251028112734368.png)

### 最佳归并树

![image-20251028122248431](/coding_images/image-20251028122248431.png)

WPL=I=O

IO总次数为WPL*2

所以要让IO次数最少，就要是的归并树WPL最小，可以联想到使用==哈夫曼树==

![image-20251028123042007](/coding_images/image-20251028123042007.png)

三路归并处理方法：找权值最小的三个（新节点也在内），同二路归并树

![image-20251028123142777](/coding_images/image-20251028123142777.png)

总IO次数：446次

**对于k叉归并，若初始归并段的数量无法构成严格的k叉归并树，比如下面那个树最后一层结点不够了，只能构成两路，则需要补充几个长度为0的“虚段”，在进行哈夫曼树的构造**

错误：

![image-20251028123630918](/coding_images/image-20251028123630918.png)

正确：

![image-20251028123617704](/coding_images/image-20251028123617704.png)

补充虚段的数量讨论：

![image-20251028123909279](/coding_images/image-20251028123909279.png)

只要让nk为整数即可

![image-20251028124019570](/coding_images/image-20251028124019570.png)
